...
class PressurePlate(gym.Env):
    ...
    def __init__(self, height, width, n_agents, sensor_range, layout, mode):
        ...
        self.observation_space_dim = sensor_range * sensor_range * 4 + 2
    ...
    def step(self, actions):
        ...
        dones = self.n_agents * [all(dones)]

        return self._get_obs_v2(), self._get_rewards_v2(dones), dones, {}
    ...
    def _get_obs_v2(self):
        ...
        for i,agent in enumerate(self.agents):
            ...
            agent_pos = (agent.x, agent.y)
            agent_obs = np.concatenate((_agents, _walls, _plates, _doors, np.array(agent_pos)))
            obs.append(agent_obs)
    ...
    def _get_rewards_v2(self, dones):
        ...
        for i, agent in enumerate(self.agents):
            ...
            door_reward = 0
            agent_reward = 0
            zone_reward = -(6 - agent.zone) - (6 - least_zone) #negative zone reward
            for door in self.doors:
            ...
            agent_reward = zone_reward * (5/1) + door_reward * (2.2) + distance_reward * (1/4) - 5
            if all(dones):
                agent_reward += 2000          
