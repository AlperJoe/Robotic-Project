import numpy as np
import torch
from MADDPGV2.agent import Agent
from typing import List, Optional
import torch.nn.functional as F
from MADDPGV2.utils import RunningMeanStd

from MADDPGV2.gradient_estimators import GradientEstimator
...
class MADDPG:
    ...
    def update(self, sample):
        ...
        target_actions = [self.agents[ii].act_target(sample['nobs'][ii]) for ii in range(self.n_agents)]
        ...
        for ii, agent in enumerate(self.agents):
            ...
            if self.policy_it % self.policy_update_freq == 0:
                agent.update_actor(
                    all_obs=batched_obs,
